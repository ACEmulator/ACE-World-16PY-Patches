REPLACE INTO `landblock_instance` (`guid`, `weenie_Class_Id`, `obj_Cell_Id`, `origin_X`, `origin_Y`, `origin_Z`, `angles_W`, `angles_X`, `angles_Y`, `angles_Z`, `is_Link_Child`)
VALUES (1897087052, 41515, 288620814, 135.584, 90.4377, 42.005, 0.20526, 0, 0, 0.978707, False); /* Anfram Mellow */
/* @teleloc 0x1134010E [135.584000 90.437700 42.005000] 0.205260 0.000000 0.000000 0.978707 */

REPLACE INTO `landblock_instance` (`guid`, `weenie_Class_Id`, `obj_Cell_Id`, `origin_X`, `origin_Y`, `origin_Z`, `angles_W`, `angles_X`, `angles_Y`, `angles_Z`, `is_Link_Child`)
VALUES (1897087053, 41526, 288620814, 139.698, 88.2701, 42.005, -0.606282, 0, 0, -0.79525, False); /* Alishia bint Aldan */
/* @teleloc 0x1134010E [139.698000 88.270100 42.005000] -0.606282 0.000000 0.000000 -0.795250 */

REPLACE INTO `landblock_instance` (`guid`, `weenie_Class_Id`, `obj_Cell_Id`, `origin_X`, `origin_Y`, `origin_Z`, `angles_W`, `angles_X`, `angles_Y`, `angles_Z`, `is_Link_Child`)
VALUES (1897087055, 28690, 288620871, 134.843, 135.569, 38.006, -0.159249, 0, 0, 0.987239, False); /* Erik Festus */
/* @teleloc 0x11340147 [134.843000 135.569000 38.006000] -0.159249 0.000000 0.000000 0.987239 */

REPLACE INTO `landblock_instance` (`guid`, `weenie_Class_Id`, `obj_Cell_Id`, `origin_X`, `origin_Y`, `origin_Z`, `angles_W`, `angles_X`, `angles_Y`, `angles_Z`, `is_Link_Child`)
VALUES (1897087056, 28686, 288620871, 134.295, 130.924, 38.006, 0.709243, 0, 0, -0.704965, False); /* Rohula bint Ludun */
/* @teleloc 0x11340147 [134.295000 130.924000 38.006000] 0.709243 0.000000 0.000000 -0.704965 */
