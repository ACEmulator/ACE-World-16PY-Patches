DELETE FROM `weenie` WHERE `class_Id` = 2812;

INSERT INTO `weenie` (`class_Id`, `class_Name`, `type`, `last_Modified`)
VALUES (2812, 'scrollflamebane2', 34, '2005-02-09 10:00:00') /* Scroll */;

INSERT INTO `weenie_properties_int` (`object_Id`, `type`, `value`)
VALUES (2812,   1,       8192) /* ItemType - Writable */
     , (2812,   5,         30) /* EncumbranceVal */
     , (2812,   8,         90) /* Mass */
     , (2812,   9,          0) /* ValidLocations - None */
     , (2812,  16,          8) /* ItemUseable - Contained */
     , (2812,  19,          5) /* Value */
     , (2812,  93,       1044) /* PhysicsState - Ethereal, IgnoreCollisions, Gravity */;

INSERT INTO `weenie_properties_bool` (`object_Id`, `type`, `value`)
VALUES (2812,  22, True ) /* Inscribable */
     , (2812,  23, True ) /* DestroyOnSell */;

INSERT INTO `weenie_properties_float` (`object_Id`, `type`, `value`)
VALUES (2812,  39,     1.5) /* DefaultScale */;

INSERT INTO `weenie_properties_string` (`object_Id`, `type`, `value`)
VALUES (2812,   1, 'Scroll of Flame Bane II') /* Name */
     , (2812,  15, 'A magic scroll.') /* ShortDesc */
     , (2812,  16, 'When learned, this spell increases a shield or piece of armor''s resistance to fire damage by 25%.') /* LongDesc */;

INSERT INTO `weenie_properties_d_i_d` (`object_Id`, `type`, `value`)
VALUES (2812,   1,   33554826) /* Setup */
     , (2812,   8,  100676651) /* Icon */
     , (2812,  22,  872415275) /* PhysicsEffectTable */
     , (2812,  28,       1548) /* Spell - Flame Bane II */;
