DELETE FROM `weenie` WHERE `class_Id` = 1875;

INSERT INTO `weenie` (`class_Id`, `class_Name`, `type`, `last_Modified`)
VALUES (1875, 'scrollacidbane', 34, '2005-02-09 10:00:00') /* Scroll */;

INSERT INTO `weenie_properties_int` (`object_Id`, `type`, `value`)
VALUES (1875,   1,       8192) /* ItemType - Writable */
     , (1875,   5,         30) /* EncumbranceVal */
     , (1875,   8,         90) /* Mass */
     , (1875,   9,          0) /* ValidLocations - None */
     , (1875,  16,          8) /* ItemUseable - Contained */
     , (1875,  19,          1) /* Value */
     , (1875,  93,       1044) /* PhysicsState - Ethereal, IgnoreCollisions, Gravity */;

INSERT INTO `weenie_properties_bool` (`object_Id`, `type`, `value`)
VALUES (1875,  22, True ) /* Inscribable */
     , (1875,  23, True ) /* DestroyOnSell */;

INSERT INTO `weenie_properties_float` (`object_Id`, `type`, `value`)
VALUES (1875,  39,     1.5) /* DefaultScale */;

INSERT INTO `weenie_properties_string` (`object_Id`, `type`, `value`)
VALUES (1875,   1, 'Scroll of Acid Bane') /* Name */
     , (1875,  15, 'A magic scroll.') /* ShortDesc */
     , (1875,  16, 'When learned, this spell increases a shield or piece of armor''s resistance to acid damage by 10%.') /* LongDesc */;

INSERT INTO `weenie_properties_d_i_d` (`object_Id`, `type`, `value`)
VALUES (1875,   1,   33554826) /* Setup */
     , (1875,   8,  100676648) /* Icon */
     , (1875,  22,  872415275) /* PhysicsEffectTable */
     , (1875,  28,       1493) /* Spell - Acid Bane I */;
